...
do 
    python3 $SCRIPTSDIR/state_cov_run.py -i $WORKDIR/$runs/out-lightftp-composed/replayable-new-ipsm-paths -s SVRM -r $runs -o $OUTDIR
    python3 $SCRIPTSDIR/state_cov_run.py -i $WORKDIR/$runs/out-lightftp-origin/replayable-new-ipsm-paths -s Manual -r $runs -o $OUTDIR

    python3 $SCRIPTSDIR/TPC_cov_run.py -i $WORKDIR/$runs/out-lightftp-composed/cov_over_time.csv -s SVRM -r $runs -o $OUTDIR
    python3 $SCRIPTSDIR/TPC_cov_run.py -i $WORKDIR/$runs/out-lightftp-origin/cov_over_time.csv -s Manual -r $runs -o $OUTDIR
