...
do 
    python3 $SCRIPTSDIR/state_cov_run.py -i $WORKDIR/$runs/out-live555-composed/replayable-new-ipsm-paths -s SVRM -r $runs -o $OUTDIR
    python3 $SCRIPTSDIR/state_cov_run.py -i $WORKDIR/$runs/out-live555-origin/replayable-new-ipsm-paths -s Manual -r $runs -o $OUTDIR

    python3 $SCRIPTSDIR/TPC_cov_run.py -i $WORKDIR/$runs/out-live555-composed/cov_over_time.csv -s SVRM -r $runs -o $OUTDIR
    python3 $SCRIPTSDIR/TPC_cov_run.py -i $WORKDIR/$runs/out-live555-origin/cov_over_time.csv -s Manual -r $runs -o $OUTDIR
...
#/home/lwfdev/MYWORK/State-Benchmark/analyse_scripts/aflnet/multi-test/state_cov_plot_run2_corpus_comparison.py
